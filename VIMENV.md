...
--  | Lint          | for .sh files runs shellcheck; for .go files it runs make vet (assumes there is a makefile with vet target)
...
Note that this .vimrc file uses vundle and the YouCompleteMe plugin; Now there is a script to set this all up (also you can set it up in a docker), see the README.me file in this project
