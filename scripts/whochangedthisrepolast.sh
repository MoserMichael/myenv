...
if [[ $1 == "-h" ]]; then
    if [[ ! -z "${SHORT_HELP_MODE}" ]]; then
        echo "for each files in git repo in current directory - show who made the last revision."
        exit 1
    fi
...
for f in $FILES; do
    ...
    who_wrote_this=$(git log --pretty='%ae %ad' $f | head -n 1)
...
for f in $FILES; do
    ...
    who_wrote_this=$(git log --pretty='%ae' $f | head -n 1)
